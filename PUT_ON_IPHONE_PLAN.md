...
**Status: ✅ Installed (2026-06-07)** — Release build signed with the free personal team (`N86QCX9R46`) and installed on `Eric's iPhone` (iOS 26.5) via CLI. Only remaining step is the one-time on-device cert trust (see below); then the home-screen icon launches standalone.

**What unblocked it:** automatic signing was off in the generated project (`CODE_SIGN_STYLE` was unset → xcodebuild treated it as manual), and `expo run:ios` does not pass `-allowProvisioningUpdates`, so the personal team could never synthesize a profile. Fix: set `CODE_SIGN_STYLE = Automatic` on both target configs in `ios/MetroTimes.xcodeproj/project.pbxproj`, flip the `LaunchAction` (⌘R Run action) in `MetroTimes.xcscheme` from `Debug` → `Release`, then build directly with xcodebuild + `-allowProvisioningUpdates` and install with `devicectl`. Note: these live under `ios/` which is gitignored and regenerated by `expo prebuild` — re-applying after a prebuild is required.

**CLI install recipe (reproduces the working install):**
```
cd mobile
...
# device UDID via: xcrun xctrace list devices
```

**Remaining one-time step (on the phone, can't be done from the Mac):** trust the developer cert — **Settings → General → VPN & Device Management → Developer App → tap the Apple ID → Trust**. Until then, launching fails with "profile has not been explicitly trusted by the user". After trusting, the icon launches standalone.
