...
**Status: ✅ Done** — verified on physical iPhone via Expo Go (Path A).
...
**Delivered:**
- Took Path A (Expo Go). Existing `pnpm expo start` dev server was already running for the project; scanned the LAN QR code from the iPhone's Camera app and opened in Expo Go.
- User-confirmed: the app loads on the physical iPhone and looks correct (parity with the simulator / `reference-web.png`).

---

## Phase 6 — Standalone install on iPhone (no dev server)
